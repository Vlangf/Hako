...
class AppDelegate: NSObject {
    ...
    static func storeTypesDictinary() -> [String: NSNumber] {
        ...
        ClipData.availableTypesString.forEach { storeTypes[$0] = NSNumber(value: true) }
        return storeTypes
    }
    ...
    @objc func selectClipMenuItem(_ sender: NSMenuItem) {
        Utilities.sendCustomLog(with: "selectClipMenuItem")
        guard let primaryKey = sender.representedObject as? String else {
            Utilities.sendCustomLog(with: "Cannot fetch clip primary key")
            NSSound.beep()
            return
        }
        guard let clip = PersistenceController.shared.fetchClip(byHash: primaryKey) else {
            Utilities.sendCustomLog(with: "Cannot fetch clip data")
            NSSound.beep()
            return
        }
    ...
    @objc func selectSnippetMenuItem(_ sender: AnyObject) {
        Utilities.sendCustomLog(with: "selectSnippetMenuItem")
        guard let primaryKey = sender.representedObject as? String else {
            Utilities.sendCustomLog(with: "Cannot fetch snippet primary key")
            NSSound.beep()
            return
        }
        guard let snippet = PersistenceController.shared.fetchSnippet(byIdentifier: primaryKey) else {
            Utilities.sendCustomLog(with: "Cannot fetch snippet data")
            NSSound.beep()
            return
        }
    ...
    private func toggleAddingToLoginItems(_ isEnable: Bool) {
        ...
        do {
            ...
            Utilities.sendCustomLog(with: "Failed to update login item: \(error)")
        }
    }
...
extension AppDelegate: NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ aNotification: Notification) {
        // UserDefaults
        Utilities.registerUserDefaultKeys()
        // SDKs
        Utilities.initSDKs()
