...
private let logger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "com.hako-app.Hako", category: "Persistence")
...
final class PersistenceController {
    ...
    private init() {
        ...
        let config = ModelConfiguration("Hako", schema: schema)
        do {
    ...
    init(inMemory: Bool) {
        let schema = Schema([ClipItem.self, SnippetItem.self, FolderItem.self])
        let config = ModelConfiguration("HakoTest", schema: schema, isStoredInMemoryOnly: true)
        do {
            container = try ModelContainer(for: schema, configurations: [config])
        } catch {
